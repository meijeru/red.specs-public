...
this argument may be an `integer!` value as well, whose binary equivalent is truncated to 8 bits,
or a `char!` or `string!` value, which is converted to `binary!` according to UTF-8; an `any-list!
value will have its components concatenated. For `any-string!` series, the `<expression>` argument,
if not a `char!` or `any-string!` value, is first `form`ed; `any-list!` values will have their
components concatenated after being `form`ed.
