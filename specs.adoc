...
mold make vector! [1 2 3] -> "make vector! [1 2 3]"
....

=== Conversion from/to other representations

The built-in function `load` has an `/as` refinement, which makes it
accept data encoded as indicated in a `word!` argument: `none` indicates
Red code (Red data); other values implemented are `jpeg png bmp tiff`
...
....
    ...
    title           string!      | 
    name            word!        | JPEG
    mime-type       block!       | length: 1  [image/jpeg]
    suffixes        block!       | length: 4  [%.jpg %.jpeg %.jpe %.jfif]
