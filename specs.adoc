...
A `word!` value does not begin with `0-9` or `'`. Words are _case-insensitive_, i.e. changing any letter in the word
into the corresponding upper- or lower-case variant does not create a different word.
...
that have lexically determined values. The full list of types of the languages
...
For several types, the available values are bound to words at program start: `none!` has `none`,
...
* internal storage: _direct types_ vs _indirect types_ and _functional types_
...
Red uses _words_ (values of type `word!`) to refer to values in much the same
way that other languages use "variables". However, in Red words *do not*
somehow "store" any values. Rather, we say that a word is _bound to a value_
in some _context_, i.e. evaluating the word in that context yields the value.
Since functions, including built-in functions and operators, are also values
in Red, the words bound to these values are similar to the "keywords" of
other languages

Thus all word values have two important properties in this regard:
their identity, i.e their spelling (disregarding upper- or lowercase),
...
are bound to. There is one _global context_ containing all words that have
passed lexical analysis as well as those that have been pre-defined by the
toolchain, and which are bound to values such as built-in functions and constants.
The words in the global context that are not pre-defined,
are bound at program start to the value "unset".
...
context, containing the field-name/value pairs of the object.
Every function (value of type `function!`) also gives rise to a context,
which will contain the pairs of formal argument name and actual argument value
to be used by the body of the function when it is executed.

The user may access the contexts through the built-in function `context-of`
_**this is still called `context?` but there is interest in changing its name**_
which can be applied to any word and will yield the object or function that
is associated with the context of the word. Asking for the global context
results in the object `system/words`.

=== Binding

Words may become bound to values as a result of:
