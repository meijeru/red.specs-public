...
which contains the internationally standardized ones in addition to some crypto ones;
...
* `string!` values: yields the result of `load` or an error
* `issue!` values: interprets up to 24 characters, if possible, as a sequence of hexadecimal numbers `0..255`;
further characters are ignored; if the number of characters is odd and less than 24, the last one is ignored
_**See https://github.com/red/red/issues/2329[issue #2329]**_
...
on the one hand, and `any-string!` and `binary!` on the other. The type of the second argument should
...
_**Functions on `vector!` and `money!` values have been requested see https://github.com/red/REP/issues/12[REP #12]
and https://github.com/red/REP/issues/69[REP #69]**_
...
their results do not correspond to a single component of the series value.

The components of `time!`, `date!`, `pair!` and `event!` values that are selected by keys (or
their equivalent indexes in the case of `time!`, `date!` and `pair!`) are not considered the
result of reflection, since they are true components. 

_**The reflection facilities requested on `vector!` and `money!` values might
also be implemented by accessors**_
...
mentioned through notes 1 to 14. The operations on values of type `pair! tuple!` and `vector!`
...
|float!        |float!^1^   |float!      |money!^1^ |time!  |         |pair!^1,2^ |tuple!^1,2,3^ |vector!^1,6^
|percent!      |            |percent!    |        |time!    |         |pair!^1,2^ |tuple!^1,2,3^ |vector!^1,6^
|money!        |            |money!^1,12^ |money!^1,4,5,14^ ||        |           |              | 
...
. percent operand not allowed
. result is integer if both operands are integer
. if both operands have a currency indicator, these must be equal
...
that evaluates to either a single component of the series or to a sub-series. For `binary!` series,
this argument will be converted to `binary!` if needed, and thus may be of any type
...
For `any-string!` series, the `<expression>` argument, if not a `char!` or `any-string!` value,
is first `form`ed; `any-list!` values will have their components concatenated after being `form`ed.
...
<a href= "https://github.com/red/red/issues/2329">2329</a>   <a href="#target-type-integer">5.4.5</a>
<a href= "https://github.com/red/red/issues/2329">2329</a>   <a href="#target-type-tuple">5.4.10</a>
<a href= "https://github.com/red/red/issues/2329">2329</a>   <a href="#target-type-binary">5.4.11</a>
