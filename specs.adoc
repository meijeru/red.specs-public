...
**`S136`**::
Similar to a `<word-literal>`, a `<refinement-literal>` is written as a `/` character
followed by one or more characters from the entire Unicode range excluding _control characters_
(notably Unicode sets C0, C1), whitespace characters and the following set: `/ \ ^ , [ ] ( ) { } " # $ % @ : ;`.
A `<refinement-literal>` *may* contain a digit `0-9` or `'` as first character after the `/`.
Refinements are _case-insensitive_, i.e. changing any letter in the refinement into
the corresponding upper- or lower-case variant does not create a different word.

Examples: `/abc`  `/Abc`  `/ABC`  `/1a`  ; the first three are the same `refinement!` value.

Note: punctuation characters from the ASCII subset that *are* allowed in refinements are: `! & ' * + - . < = > ? _ `` `| ~`.

==== `issue!`

**`S137`**::
Similar to a `<word-literal>`, an `<issue-literal>` is written as a `#` character
followed by one or more characters from the entire Unicode range excluding _control characters_
(notably Unicode sets C0, C1), whitespace characters and the following set: `/ \ ^ , [ ] ( ) { } " # $ % @ : ;`.
An `<issue-literal>` *may* contain a digit `0-9` or `'` as first character after the `/`.
Issues are _case-insensitive_, i.e. changing any letter in the refinement into
the corresponding upper- or lower-case variant does not create a different word.

Examples: `#abc`  `#Abc`  `#ABC`  `#10FFFF`  ; the first three are the same `issue!` value.

Note: punctuation characters from the ASCII subset that *are* allowed in issues are: `! & ' * + - . < = > ? _ `` `| ~`.
...
Selection by a key of the wrong type or wrong value leads to a `none` result
on extraction for values of all indirect type except `email!` where an error is produced.
On changing, an error is produced for values of all types whether direct or indirect,
except in the case of `map!` values, where a non-existing key will be added to the map. 
