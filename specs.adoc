...
Additionally, values of `series!` types may be indexed by `logic!` values, where `true` yields
the first component and `false` the second component.
Furthermore, bitsets admit indexing by characters and strings, with the result being `true`
if the given character(s) is/are found in the bitset, and images may also be indexed by pairs
as coordinates, in accordance with the row-oriented storage of the pixels.
Built-in functions for indexing are `pick` for extraction and `poke` for changing, with the
...
Built-in functions for selection are  `select` for extraction and `put` for changing, with the
following definitions:
....
select <source> <key>
...
|pair!      |1 2          |pick        |x y                                 |
|tuple!     |1 .. 12      |pick        |                                    |
|any-block! |integer! logic!    |pick poke   |any-type!                           |select put
|vector!    |integer! logic!    |pick poke   |integer! char!                      |select^1^
|string!    |integer! logic!    |pick poke   |char! any-string! binary!           |select^1^
|email!     |integer! logic!    |pick poke   |char! any-string! binary! host user |select^1^
|file! url! |integer! logic!    |pick poke   |char! any-string! binary!           |select^1^
|binary!    |integer! logic!    |pick poke   |integer! char! any-string! binary!  |select^1^
