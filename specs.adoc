...
_**An alternative, indirect syntactical representation,
...
Values that are sequences admit indexing by integers (and images also by pairs as coordinates).
...
and `v/i: x` is equivalent to `poke v i x`.

As stated in section _**Atomic/Composite types**_, component selection
in values of direct types cannot be used to change the component, only to extract it.
That is, only `<path>` and `pick` are allowed for these values.

Values of type `time! tuple! email!` and `image!` also admit component selection
...
....
type         indexed        built-in           components                           built-in
             components     functions          selected otherwise                   functions

time!        /1 /2 /3       pick (1..3)        /hour /minute /second                --
pair!        /1 /2          pick (1..2)        /x /y                                --
tuple!       /1 .. /12      pick (1..length)   --                                   --
any-block!   /<integer>     pick poke          /<selector> (any-type!)              select put
vector!      /<integer>     pick poke          /<selector> (element type)           select (put see issue #1960)
any-string!  /<integer>     pick poke          /<selector> (any-string! binary!)    select (put see issue #1960)
+ email!                                       /host /user                          --
binary!      /<integer>     pick poke          /<selector> (any-string! binary!)    select (put see issue #1960)
image!       /<integer>     pick poke          /size /rgb /alpha /rgba              --
             /<pair>        pick poke
bitset!      /<integer>     pick poke          --                                   --
map!                                           /<selector> (any-word! any-string!   select put
                                                     number! char! pair! tuple!)
...
Values of types `lit-word! set-word! get-word! refinement!` and `issue!`
have the same binding as the word with the same symbol.
...
an index (row number) in the table.
...
`system/words`. The index of the word in its context may be obtained through
...
**`E105`**:: Evaluating a `<word>:` outside an `<object-spec>` or a `<map>` (including
the `<spec>` in `make map! <spec>`) has the effect that the `<word>` in its context
...
.. Bind the body of the function to the context (see sub-section _**Built-in bind function**_).
...
in the `<argument-spec>`. Note also that for `action!` values,
