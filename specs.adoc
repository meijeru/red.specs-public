...
A `<` character cannot be part of a `<word-literal>` if it follows anything but other `<` character.
...
However, the combinations `#<>` and `#<...>` with any characters in between the `<` and `>` character are not allowed.
