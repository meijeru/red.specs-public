...
The `equal?` test ignores case differences in spelling of words and strings, and
in the case of floating point numbers, a very small difference in actual value.

For values of `any-block!` and `any-object!` type, the `strict-equal?` and `equal?`
tests apply pairwise to the components and if these are also of `any-block!` or
`any-object!` type, there is recursion involved, with cycle detection.

Finally, for the `equal?` test, the requirement of equal type is relaxed for four
groups of types. Within each group of types, `equal?` will allow any two types
