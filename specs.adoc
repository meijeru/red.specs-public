...
on each subsequent iteration the occurrence of the `<word>` will refer to the
series at the next position (see `next` function in <<series-positioning-and-navigation,section 9.2>>);
after the last iteration, the `<word>` will again refer to the original series value.
