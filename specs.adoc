...
load "1 2 3"              -> [1 2 3]              ; load produces a block for more than one value
...
Moreover, with the `/as` refinement, it will accept data encoded as indicated
in the `word!` argument: `none` indicates Red code (Red data), other values
implemented are `jpeg png bmp tiff` which will result in an `image!` value.
The corresponding decoding and encoding functions are stored in `system/codecs`.
