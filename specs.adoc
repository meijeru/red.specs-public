...
for other types, `reduce` yields the result of evaluating the argument.
...
Applied to an argument of any other type, `compose` yields the result of evaluating the argument.
...
All three functions `reduce compose` and `collect` have a refinement `/into` with
a block value as argument; if present, the components of the result will be inserted into
this block.
