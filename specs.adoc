...
`*` means zero or more instances. The comment to any production rule,
which starts after the `;` on the line, states the type of the single
or grouped values generated by this rule. Any non-terminal that is not
further defined in the grammar is explained in the individual sub-sections
of section _**Lexical structure of values**_ hereafter.
...
`<integer>` will stand for " `<expression>` evaluating to an `integer!` value".
Decorated non-terminals like `<true-block>` will stand for `<block>`
...
or as a _binding_ of a word to a value.
...
be modified to: "execution of a Red program is tantamount to evaluating each of its
...
in section _**Evaluation rules**_. Evaluation of the operands of operators has precedence
...
handling this communication. Likewise, values of type `action!` and `native!` cannot
be made by the user, but all necessary values of those types are made by
the toolchain itself as part of its initialization.
...
**`S112`**::
A `<float-literal>` is written as a signed floating point number in the range of the IEEE 754 binary64 format,
in decimal notation. Leading zeroes are allowed, as well as `'` signs for separation.
...
This is treated in rule `S130` and in section _**Errors**_.
...
As stated before, the equivalent to `<word>: <expression>` is `set '<word> <expression>`.
...
as explained in section _**Selection by "key"**_.
.. Otherwise, the path is in error.
. If a further `<selector>` is present, use the intermediate result just obtained as 
evaluated `<path-head>` and go to step 2, otherwise finish: the result of the evaluation is
the intermediate result just obtained.
...
.. If the `<selector>` is a `<word>`, and the evaluated `<path-head>` is of
indirect type (except `image!` and `email!`), an intermediate result is obtained by
applying the built-in function `select` with as arguments the evaluated
`<path-head>` and `<selector>`.
.. If the `<selector>` is a `<word>`, and the evaluated `<path-head>` is of
direct type, `image!`, `email!` or `event!`, an intermediate result is obtained
as explained in section _**Selection by "key"**_.
.. Otherwise, the path is in error.
...
the intermediate result as result of the evaluation.
...
position of the component found as as explained in section _**Selection by "key"**_.
.. Otherwise, the path is in error.
. If a further `<selector>` is present, use the component at the position just determined,
as evaluated `<path-head>` and go to step 2, otherwise finish:
replace the component at the position determined, by the value obtained by evaluating the next expression.
An error occurs if no value is following or if the value obtained is unset.
...
Moreover, the equivalent to <path>: <expression> is set '<path> <expression>.
...
The evaluation of an `action!` value then proceeds as follows:
...
two arguments and no refinements. Therefore, the evaluation proceeds as for the
...
The evaluation of a `native!` value then proceeds as follows:
...
functions are as follows. Result types are only indicated if different from argument types.
....
...
negate                   1          number! time! pair!
absolute round           1          number! time! pair!
max min                  2          scalar!
....
...
`pair! tuple! vector!` take place component-by-component where possible.

The functions `max min` are also defined for `series!` arguments. See section
...
it is also defined for `series!` arguments. See section _**Series manipulation**_.
...
The following built-in functions have been described in section _**Components of values**_:
...
set a _new-line marker in the value slot of the current component of
