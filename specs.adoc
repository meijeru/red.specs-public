...
. otherwise, the argument will be evaluated _passively_, i.e. according to the appropriate
rule of <<evaluation-rules,section 7>>, with the exception that resulting values
of type `any-function!` (excluding `op!`) will not be applied 
...
Red currently has a basic set of functions to work directly
...
https://www.ietf.org/rfc/rfc3986.txt[RFC3986]. The functionality
offered by ports will be more complete than the basic I/O currently provided.
...
with a wide variety of external resources. A prototype port value is contained in `system/standard/port`.
As also shown in <<components-of-values,section 5.6>> the fields are:
[cols="1,1,2", options="header"]
|===
|name        |type           |content
|`spec`      |`object!`      |represents a resource, i.e. a URI
...
of resource, e.g. they will be different for `http:` than for `udp:`.
...
It will be seen that the field names of the `spec` object are identical to those used in RFC3986.
...
file or url respectively as indicated above; the `block!` should
contain key/value pairs as for an object.
