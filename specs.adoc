...
The actions applicable to ports are described in <<port-operations,section 11.4.2>>;
besides I/O actions proper, these comprise most of the series actions as already
described in <<operations-on-values-of-indirect-types,see sections 9.2/3>>.
These actions differ according to the _type_ of resource, which RFC3986 calls its _scheme_.
For certain schemes, Red provides built-in implementation of the actions. For other
schemes, the user can provide the implementation in the form of Red functions.
