...
which initially contains the internationally standardized ones (ISO 4217) in addition to
...
A `<ref-literal>` consists of a `@` sign followed by any number of characters other than
...
_**See https://github.com/red/red/issues/4378[issues #4378], https://github.com/red/red/issues/4389[#4389]
...
truncated integer division (`//` or `divide-strict`), remainder (`%` or `remainder`)
and modulo (`%%` or `modulo`). Enquiry functions are: `sign?`, `positive?`, `negative?`,
`zero?`, `even?`, `odd?` and `NaN?` ("not a number", see <<float,section 5.2.2>>).
...
|`+ - * / %`           |2     |`vector!`             |`number!`          |`vector!` ^3^
|`+ - * / %`           |2     |`number`              |`vector!`          |`vector!` ^3^
|`+ - * / %`           |2     |`vector!`             |`vector!`          |`vector!` ^4^
|`+ - * / % %%`        |2     |`money!`              |`integer! float!`  |`money!`
|`+ - *`               |2     |`integer! float!`     |`money!`           |`money!`
|`+ - / % %%`          |2     |`money!`              |`money!`           |`money!` ^5,6^
|`+ - * / % %%`        |2     |`time!`               |`number!`          |`time!`
|`+ - * / % %%`        |2     |`number!`             |`time!`            |`time!`
...
|`+ -`                 |2     |`date!`               |`integer!`         |`date!`
|`+`                   |2     |`integer!`            |`date!`            |`date!`
|`-`                   |2     |`date!`               |`date!`            |`integer!` ^7^
|`+ -`                 |2     |`date!`               |`time!`            |`date!`
|`+`                   |2     |`time!`               |`date!`            |`date!`
|`+ - * / % %%`        |2     |`time!`               |`time!`            |`time!` ^6^
|`+ - * / % %%`        |2     |`pair!`               |`pair!`            |`pair!`
|`+ - * / % %%`        |2     |`tuple!`              |`tuple!`           |`tuple!`
|`**`                  |2     |`number!`             |`integer! float!`  |`integer!` ^8^ `float!`
...
|`NaN?`                |1     |`number!`                                |`logic!`
|`sign?`               |1     |`money! number! time!`                   |`integer!`^9^
...
|`negate`              |1     |`money! number! time! pair!`             |same as operand ^10^
|`absolute round`      |1     |`money! number! time! pair!`             |same as operand ^10^
|`sum`                 |1     |`any-list!` ^11^ `vector!`               |same as individual numbers +
(refer to table above if not all numbers have the same type)
|`average`             |1       |`any-list!` ^11^ `vector!`             |see note ^12^
|===
...
. the result is truncated to an integer by removing the decimal fraction
...
The built-in function `load` will convert textual input (a `string!` value or its binary
UTF-8 encoded equivalent), specified as its argument, to one or more Red values
by calling upon the lexical/syntactic analysis facilities of the toolchain.
The result is either one value or a block of values. If called with an argument (source)
that is a `file!` or `url!` value, `load` will read the content of the source
and -- if the representation cannot be otherwise determined
(see <<conversion-fromto-other-representations,next section>>) -- will treat it
as described above.
...
with a `word!` argument: the value `none` indicates Red code (molded Red values)
...
format and decode it to yield an `image!` value, and `save` will take an `image!`
value and produce the encoded binary value. The necessary decoding and encoding
routines are stored in `system/codecs`. This is a block of pairs: `<word><object>`,
where the `<word>` indicates the encoding e.g. `jpeg`, and the corresponding `<object>`
has the following content (example for the `jpeg` case):
[cols="1,1,2", options="header"]
|===
...
The codec for JSON data (see https://www.json.org/) has the following
object in `system/codecs`:
[cols="1,1,2", options="header"]
|===
    |key             |value type   |content
...
For comma separated values (CSV) the MIME type is `text/csv` and the extension is `%.csv`.
...
<a href= "https://github.com/red/red/issues/2650">2650</a>  A <a href="#equality-comparisons">5.8.1</a>
<a href= "https://github.com/red/red/issues/2671">2671</a>  F <a href="#char">5.2.6</a>
...
<a href= "https://github.com/red/red/issues/3571">3571</a>  R <a href="#file-system-functions">11.3.2</a>
...
<a href= "https://github.com/red/red/issues/4378">4378</a>  N <a href="#creation-of-values-make">5.3</a>
<a href= "https://github.com/red/red/issues/4389">4389</a>  N <a href="#creation-of-values-make">5.3</a>
