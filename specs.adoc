...
A `<symbol-literal>` by itself is not a lexeme, and therefore does not generate a value of the
language. Its use is in explaining the common characteristics of values of
several types, including `<word-literal>` as will be seen in <<code-word-code,section 5.2.5>>.
...
A `<tag-literal>` value is written as one or more characters, not starting with `<`, `=` or `>`, enclosed in `< >`.
...
**`S136`**::
A `<refinement-literal>` is written as a `/` character followed by a `<symbol-literal>`.
...
**`S137`**::
An `<issue-literal>` is written as a `\#` character followed by a `<symbol-literal>`.
...
* For sequences, a case-insensitive `find` action is performed on the components using the key,
...
corresponding to the given key, or if there is no such key in a `map!` value the result
is `none`, and in case of an `object!` or `error!` value the result is an error.
