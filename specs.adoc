...
system/   object!
    ...
    words       object!             global context, see <<contexts,section 6.2>>
    platform    function!           returns a word identifying the operating system
    catalog/    object!
        datatypes
        actions
        natives
        accessors      block!       see <<selection-by-key,section 5.6.2>>
        errors         object!      see <<code-error-code,sections 5.3.7>>, <<composition,12.1.1>>
    state/      object!
        interpreted?   function!    returns TRUE if called from the interpreter
        last-error     error!       
        trace          integer!     
    modules     block!              
    codecs      block!              see <<conversion-from-to-other-representations,section 11.2>>
    schemes     object!             TBD
    ports       object!             TBD
    locale/     object!             
        language       
        ...
        locale         
        locale*        
        months         block!       see section <<code-date-code,5.2.14>>
    ...
    options/    object!             
        ...
        thru-cache     file!         see section <<file-and-url-i-o,11.5>>
        args           
        do-arg         
        debug          logic!
        secure         
    ...
    script/     object!
        ...
        args           
    standard/   object!
       header          object!        see <<metadata-for-the-toolchain,section 17>>
       error           object!        see <<composition,section 12.1.1>>
    lexer       object!               run-time lexer program + data
    console/    object!               console program + data
        prompt         string!        see <<console-functions,section 11.7>>
        result         string!        see <<console-functions,section 11.7>>
        history        block!         previous inputs
        ...
    view        object!               see <<gui-system,section 13.3>>
    reactivity  object!               see <<reactivity,section 13.4>>
++++
...
direct type                             4.2.2
disk-cached remote file access          11.5
...
remote file access (disk-cached)        11.5
