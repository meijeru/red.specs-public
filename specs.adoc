...
as defined in <<code-char-code,rule `S116`>> and `<base64-char>` is a single character
from the set `A-Z a-z 0-9 + /`; the individual elements within the `#{ }` brackets
(`<base2-byte>`, `<hex>` or `<base64-char>`) may be separated from the brackets
...
whenever a component of a series value which is the value of a field, or a
(sub-)component of that field -- at any level of nesting -- changes its value.
In order to permit the triggering of the latter function, all series values
...
referred to in <<additional-information,section 18>>.
