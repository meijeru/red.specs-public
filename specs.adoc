...
are the words of all the `set-word!` values that are (first-level) components
of the `<object-spec>`. The corresponding values are set to the unset value.
The `<object-spec>` is bound to this context (see section _**Built-in bind function**_).
The bound block is then executed. The `class` property of the newly created object
is set to a unique integer.
...
Note that the `logic!` values `true` and `false` are not allowed as keys. These
words will be considered as `word!` values.
...
context, containing the field-name/value pairs of the object. Every _error_
(value of type `error!`), which is in fact a specialized object,
likewise gives rise to a context. Every function 
