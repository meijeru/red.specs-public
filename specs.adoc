...
. Start from the first path component. This `<path-head>` is a `<word>`. Evaluate
the `<path-head>`.
...
.. If no actual refinements are present, evaluate as many subsequent values as correspond
...
not matched by an actual refinement, as well as each `<word>` of an `<argument-name>`
