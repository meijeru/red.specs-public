...
or a `char!` or `string!` value, which is converted to `binary!` according to UTF-8. For `any-string!`
series, the `<expression>` argument, if not a `char!` or `any-string!` value, is first `form`ed.
