...
involke `load` (see _**Input/output**_) first , and then evaluate the result. If the argument is
...
* making values (see section _** **_)
* converting values (see sections _** **_ and _** **_)
* enquiry
* comparison (see section _** **_)
* evaluation and binding
* input/output (see section _** **_)
* exception handling

The letter in front of each word gives the type: `A` for `action!`, `N` for `native!`,
`O` for `op!`, `R` for `routine!` and `F` for `function!`
