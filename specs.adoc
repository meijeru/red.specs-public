...
* creating them with the help of other ones (`make`)
* converting values to related ones of other types (`to`)
* changing their type without changing their content (`as`)
...
and `any-string!`. The built-in casting function `as` is defined with arguments
`<type> <spec>`. Here `<type>` is a type or example value ("prototype") and `<spec>`
is the value to be given a new type. The type of this value should be in the same group
as the (type of the) first argument. Note the absence of `hash!` from the first group,
