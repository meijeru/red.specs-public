...
detail below, any _word_ may be _refer_ to a value in some _context_.
...
occur in a Red _program_ which, as stated before, is a sequence of values.
THe user may furthermore use and interpret these values, when considered as _data_,
in ways of their own, and thus create _dialects_ or _Domain Specifi Languages
(DSLs_)..

In fact, Red itself contains
a number of dialects where blocks of data are interpreted in a specific way:
the _function spec dialect_ , the _object spec dialect_ , the _parse dialect_,
the _view dialect_, the _draw dialect_ which includes a _shape dialect_ and more.

In fact, the standard interpretation of Red values might be called the _do dialect_,
...
bound to. Something words _don't_ have is a restriction on what values they
can refer to. In Red, values are strongly typed, but words, when used like
variables, are not.

**TBD: CLEAN UP
In the case of non-immediate values, more than one word can refer to the same
value. For `immediate!` or `internal!` values, evaluating a word may yield
an equal result, for `series!`, `object!`, and some other types, they are
