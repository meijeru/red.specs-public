...
a time of day -- a non-negative `time!` value from 0:00:00 to 24:00:00 -- optionally
combined with timezone information; the latter is a signed time value between
-15:45:00 and +15:45:00 in multiples of 00:15:00. Note that actual timezones
vary between -12:00:00 and +14:00:00.
...
This represents a date and time with timezone 0:00 (UTC).
...
which is interpreted in the current century unless that would make it to be in the future,
in which case it is interpreted in the previous one.
...
For `<hmsd> <hours>` and `<minutes>` see rule `S113`; while `<hours>` may be
a single-digit number, `<minutes>` must have 2 digits here _**see issue #2854**_; `<hhmm>` stands for
hours and minutes as in the ISO representation. The timezone value is used modulo 16:00
and is rounded down towards zero in multiples of 15 minutes.
