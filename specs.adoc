...
a time of day expressed as a UTC time value with optional timezone information;
the UTC time is a non-negative `time!` value and the timezone is a signed `time!` value.

A `<date-literal>` uses either a very restricted form of the representation defined
by the _ISO 8601 date and time standard_ or a native Red format, as explained
in the following lexical/syntactic rule.
...
with overflow resulting in increasing the date.

For `<hours>` and `<minutes>` see again  rule `S113`;
...
The value of `<zone-offset>` is used modulo `16:00` and is rounded down in multiples of `00:15`;
thus the timezone varies between `-15:45` and `+15:45`
...
* for `date!` values: `1 ~ year 2 ~ month 3 ~ day 4 ~ zone 5 ~ time 6 ~ hour 7 ~ minute
8 ~ second 9 ~ weekday 10 ~ yearday = julian 11 ~ timezone 12 ~ week 13 ~ isoweek`
...
component is the week number according to the ISO 8601 standard; setting the `timezone` component
results in a new `date!` value with the corresponding `zone` component and a changed `time` component
