...
A `<word-literal>` is a `<symbol-literal>` that does not begin with `0-9` or `'` or an unmatched `<`
(exception: `<`, `<=` and `<<` are `<word-literal>` s), and does not contain `\`.
Furthermore, the combination `<>` may only occur on its own,
while `<...>` with any characters in between the `<` and `>` character is a value of type `tag!`
