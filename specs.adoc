...
each pixel, as follows: B in bits 0..7, G in bits 8..15, R in bits 16..23, A in bits 24..31. When specifying or retrieving
a single pixel (see also section 5.6), these values are encoded in a 3 or 4-element `tuple!` value as follows:
first 3 elements are R, G and B, and last element, if present, is 255 - A. Thus, with a 3-element tuple,
the default value for A is obtained as `255` (fully opaque).

If `<image-spec>` is `<pair>`, the image is created with the given dimensions, and with all pixels having color
`255.255.255` and transparency `255`. If a `<tuple>` is specified, this determines the color and transparency
of all pixels as stated above. If a single `<binary>` is specified, this should contain the sequence of colors of all pixels
(three bytes per pixel, stored by horizontal line), the transparency being `255`. The second `<binary>`, if present,
contains the transparency (one byte per pixel, in the same ordering). If the previous item in the specification
was a 4-element tuple, this `<binary>` is ignored. 
...
trim <series>                 spaces removed from string                   at head
                              or none from block or hash value             see also issues #2521, 2783
