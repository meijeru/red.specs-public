...
|`tuple!`         |e.g. color in RGB or other scheme, IPv4 address
...
|`bitset!`        |sequence of `logic!` values, used e.g. to model sets of
...
The function `to-logic` yields `true` for any argument value except `false` or `none`. Note that
