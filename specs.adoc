...
equivalent) to one or more values by calling upon the lexical/syntactic analysis
facilities of the toolchain.
...
The built-in function `save` will apply `mold` to its second argument
and append the result to its first argument if that is a string, or
create a UTF-8 encoded `binary!` value out of the result
which it will append to its first argument if that is a `binary!` value.
_**See issue #2668**_

=== Conversion from/to other representations

The built-in function `load` has an `/as` refinement, which makes it
accept data encoded as indicated in a `word!` argument: the value `none`
indicates Red code (Red data); other values implemented are `jpeg png bmp tiff`
each of which will result in an `image!` value. The corresponding
decoding functions are found via `system/codecs`.
...
The built-in function `save` also has an `/as` refinement, which makes it
emit data encoded as indicated in a `word!` argument: the value `none`
indicates Red code (or Red data) as "molded" string; other values implemented
are `jpeg png bmp tiff` each of which will result in a `binary!` value
which encodes an `image!` value using an encoding function found in `system/codecs`.
...
device which can be indicated by a `file!` or `url!` value. The content
...
will be accounted for. If the refinement `/binary` is used, the content
will be read as a `binary!` value and not be decoded.
