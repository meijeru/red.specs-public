...
of documentation will be referred to (e.g.: the GUI documentation that can be
found https://doc.red-lang.org/en/gui.html[here])
...
in the https://github.com/red/red/blob/master/README.md[README] file of the
https://github.com/red/red[Red repository on Github],
which contains the full source code of the toolchain.
...
A `word!` value does not begin with `0-9` or `'`. Punctuation characters from the ASCII subset that *are* allowed
in words are: `! & ' * + - . < = > ? _ `` | ~`. Words are _case-insensitive_, i.e. changing any letter in the word
...
object!         collection of word-value pairs, i.e. a context in which the words
                are bound, and refer to the corresponding values 
