...
A `<tag-literal>` is written as one or more characters, not starting with a whitespace character, `<`, `=` or `>`,
enclosed in `< >`. Characters `"` and `'` are allowed but must each be properly paired. Within such pair,
the other one may occur freely, thus `"...'..."` and `'..."...'` are allowed.
