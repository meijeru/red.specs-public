...
stops after the first one that is satisfied, if any. If none of the conditions is satisfied,
an error is raised.
...
The functions `if`, `unless` and `switch` without `/default` yield `none` if
