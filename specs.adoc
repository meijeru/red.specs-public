...
however they behave slightly differently as noted below.
...
6.3.6>>). The bound block is then _executed_. It is an error for the bound block to contain a `return`
or `exit` as first level component. However if the object is made by applying `context`
or `object`, the presence of `return` followed by an expression yields the value of that expression,
without an object being created._**See issue #3362**_
...
|type       |indexed components |built-in functions |key values or types   | built-in functions
|time!      |1 2 3        |pick        |hour minute second                  |
|date!      |1 .. 10      |pick        |date year month day zone time hour minute second weekday yearday julian timezone week isoweek |
|pair!      |1 2          |pick        |x y                                 |
|tuple!     |1 .. 12      |pick        |                                    |
|any-block! |integer!     |pick poke   |any-type!                           |select put
|vector!    |integer!     |pick poke   |integer! char!                      |select (1)
|string!    |integer!     |pick poke   |char! any-string! binary!           |select (1)
|email!     |integer!     |pick poke   |char! any-string! binary! host user |select (1)
|file! url! |integer!     |pick poke   |char! any-string! binary!           |select (1)
|binary!    |integer!     |pick poke   |integer! char! any-string! binary!  |select (1)
|image!     |integer! pair! |pick poke |size rgb alpha argb                 |
|bitset!    |integer! char! |pick poke |                                    |
...
negate                1     number! time! pair!                   same as operand see REP #10
absolute round        1     number! time! pair!                   same as operand see REP #10
...
vector!               vector!  vector!  vector!                                        vector!
 ...
 (9) only add allowed
(10) only subtract allowed; result is number of days;
     for exact difference use built-in function difference
....
...
it yields a `time!` value as the result of subtracting one date and time from the other. 
...
complement     1    integer! bitset! typeset! binary! see REP #10
