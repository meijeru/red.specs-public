...
A `<money-literal>` consists of an optional sign (`+` or `-`) followed by `$` and an unsigned integer
(<<integer,5.2.1>>) or float (<<float,5.2.2>>) literal. Float literals may not use the exponent notation,
