...
.. If the result is a value of composite type, the next element is a `<selector>`
which will yield a component of the composite value as described
in the next step.
.. If the result is a value of `any-function!` type,
evaluate the result according to the rules for values of function types
(see section xxx).
