...
* `decompress` takes a binary argument and decompresss it according to GZIP (default),
...
* accessors, which have the form of selection keys (see <<selection-by-key,section 5.6.2>>)
...
* those testing arithmetic properties (see <<basic-arithmetic-operations,section 8.1>>):
`NaN? positive? negative? zero? even? odd?`
* those testing properties of series: `empty? single? head? tail? last?` (see
<<series-positioning-and-navigation,section 9.2>> and `new-line?` (see
<<conversion-fromto-textual-representation,section 11.1.1>>)
* `complement?` which applies to `bitset!` values (see <<bitset,section 5.3.5>>)
* those testing properties of files: `exists? dir?` (see <<file-system-functions,section 11.3.2>>)
and ports: `open?` (see <<port-operations,section 11.4.2>>)
...
For `series!` values, one has furthermore `length?` and `offset?` as defined
in <<series-positioning-and-navigation,section 9.2>>.

The function `sign?` yields the sign of a `number!` value as `+1`, `0` or `-1`.

The functions `size?` and `suffix?` are defined on `file!` values (see <<file-system-functions,section 11.3.2>>).
...
For values of types `date! email! event! image! pair! time!` which admit (pseudo-) component selection
...
For `email!`, `image!` and `event!` values, the keys may be considered reflectors since
their results do not directly correspond to an addressable component of the value.
...
* <<enquiry-and-reflection,enquiry-and-reflection>>, i.e. properties of types and values
