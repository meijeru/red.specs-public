...
value and produce the encoded binary value. For the JSON and CSV formats,
...
according to UTF-8; `save` will accept a data argument of any type and a `file!`,
`url!` or `none!` argument as destination, where `none` results in a `string!` value.
...
has the following fields: `name`, `mime-type`, `suffixes`, `encode` and `decode`.
