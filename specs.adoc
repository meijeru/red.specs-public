...
a pointer to a context (see next section), an index in a symbol table,
...
The above goes also for values of types `lit-word! set-word! get-word!
...
refer to. One can think of it as being composed of two columns,
...
The built-in function `bind` will change the binding of a word or
...
refinement!` and `issue!`, and will treat them at any depth within
the block and its sub-blocks (including values of type `paren! path!
lit-path! set-path! get-path!` and `hash!`). It will set the context
of the word(s) to the given context, which is supplied in the
form of a word (whose context will be used), or of an object or error value
or a function.
...
**`E100`**:: For all values of passive types evaluation yields the value itself. This is called the **identity rule**.
...
`lit-word! get-word!` and `refinement!`) occurring in the `<function-spec>`.
Make all these words initially refer to `none`.
.. Evaluate as many subsequent values as correspond
...
as in the previous sub-step, evaluating the necessary values and inserting them in the context.
