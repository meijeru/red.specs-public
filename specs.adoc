...
An expression groups one or more values, and may be formed in two ways: as an _application_
of a (prefix) _function_ or as an _infix expression_ which uses an _operator_.
...
<operand>         ::= <value>
                    | <application>
<application>     ::= <prefix-function> <argument>*
<prefix-function> ::= <action> | <native> | <function> | <routine>
<argument>        ::= <expression>
....
...
over function application; also, as suggested in rule `S101`, operators are strictly _left associative_,
...
blocks of code. Care has to be taken that the argument is a valid Red value,
