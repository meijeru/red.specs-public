...
also, as suggested in rule `S101`, operators are strictly _left associative_,
...
will treat the predefined words, referred to in the previous paragraph, as `word!` values,
...
Note: A separate datatype `bigint!` (see <<code-bigint-code,section 5.3.13>> is available for
...
(see <<other-types,sections 5.3.14>> and <<target-type-code-date-code,5.4.13>>).
...
If `< block>` is omitted, the vector components are initialized to "zero" (`0` for `integer!`, `0.0` for `float!`,
`0%` for `percent!` and `#"^(null)"` for `char!`).
...
Note: an `<integer>` is not allowed in place of the `<tuple>`._**See https://github.com/red/red/issues/2883[issue #2883]**_
...
to the unset value. The `<object-spec>` is _bound_ to this context (see <<built-in-code-bind-code-function,section
6.3.7>>). The bound block is then _executed_. _**See https://github.com/red/red/issues/3362[issue #3362]**_
...
<function-body> ::= <block>
....
For `<typeset-element>` see <<code-typeset-code,rule `S124`>>. Note that for a program to be compiled,
...
_**See https://github.com/red/red/issues/2572[issue #2572]**_
...
(`%<hex-byte>`, see sections <<code-file-code,5.2.8/9/10 and 12>>) into a `string!` value
in which the characters have been decoded
* `debase`and `enbase` have been described in <<code-binary-code,section 5.2.12>>
* `decompress` TBD
...
This is treated in <<code-port-code,rule `S135`>> and in section <<ports,11.4>>.
...
applies, _except_ if the first argument is a function call with a literal first argument,
and there is no second argument provided.
...
|find <map> <key>       |yield true if key indicated by <key> exists, else none +
...
`in` (<<built-in-code-in-code-function,section 6.3.8>>). Of these,
`in` and `select` are also defined on errors and ports.

The following built-in functions are also defined on objects, errors and ports,
with the following semantics:
[cols="1,2", options="header"]
|===
|function                  |operation
|copy <any-object>         |yield a copy of the object, error or port
...
The built-in function `save` will apply `mold/all` to the expression which is its
second argument. If the destination specified as its first argument is a string,
it will append the result of `mold/all` to that string. If the destination
is a `binary!` value, it will first create a UTF-8 encoded `binary!` value
out of the result of `mold/all` and append that value to the destination .
...
all series actions <<operations-on-values-of-indirect-types,see sections 9.2/3>> are allowed on ports.
...
UTF-8 encoding                          <a href = "#textual-structure">3.1</a>, <a href = "#target-types-in-code-any-string-code">5.4.3</a>, <a href = "#target-type-code-char-code">5.4.7</a>, <a href = "#target-type-code-binary-code">5.4.10</a>, <a href = "#conversion-fromto-textual-representation">11.1.1</a>, <a href = "#file-and-url-io">11.3.1</a>
...
<a href= "https://github.com/red/red/issues/324">324</a>   <a href="#code-function-code">5.3.8</a>
...
<a href= "https://github.com/red/red/issues/2572">2577</a>   <a href="#code-map-code">5.3.11</a>
...
<a href= "https://github.com/red/red/issues/2668">2668</a>   <a href="#conversion-fromto-textual-representation">11.1.1</a>
