...
are all the `set-word!` values that are (first-level) components of the `<object-spec>`.
The corresponding values are set to the unset value. The `<object-spec>` is bound
to this context (see section _**Built-in bind function**_). The bound block is then
executed. The `class` property of the newly created object is set to a unique integer.
...
`arg2` and `arg3` will also be none`
...
which is the error message, or a block, which needs to be bound 
to the context of the error value, in order for the values of
`arg1` to `arg3` to be inserted in the error message.

When the error value is produced by the system, its `id` field
is bound such that it refers to the right error message.
Thus the following code will produce the full message for an error
value, say `err`:
....
either err/arg1                        ; test if insertion is necessary
[
    bind (get error/id) (in err 'id)   ; yields a block
][
    get error/id                       ; yields a string
]
....
