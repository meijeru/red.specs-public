...
Notes: some Operating Systems use _directory_, others use _folder_. Red
consistently uses directory. A `file!` value may either indicate
a file or a directory. The device name, if any, and the names
in the directory hierarchy are separated by a `/` character.
Some operating systems use the `\` character for the same purpose.
...
and produce the equivalent Red `file!` value.

The Red file convention is that a file name starting with
a `/` character represents an _absolute path_, and a file name
ending in a `/` character represents a directory.

The current working directory is stored in `system/options/path`.

The following built-in functions deal with the file system:

* `exists?` will yield `true` if its `file!` argument exists in the file system,
and `false` otherwise
