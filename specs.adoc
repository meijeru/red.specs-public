...
with external resources;0 they are specialized objects with a fixed number of fields,
that contain the necessary information for handling the communication
with a wide variety of such resources. A prototype error value is contained in `system/standard/port`.
