...
<routine-argument> ::= <word> [<type-literal>] <argument-doc>°
...
and return spec must have a single type specified. The `<word-literal>`
must refer to an existing Red type. The `<routine-body>`
