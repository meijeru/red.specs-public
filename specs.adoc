...
<typeset-element> ::= <typeset-name> | <type-name>
....
where `<type-name>` is one of the pre-defined type names.

Examples: `number!` is `make typeset! [integer! float! percent!]`,
`scalar!` is `make typeset! [char! number! time! pair! tuple!]`.     
...
....
The components of `<block>` should all have the same type `char! integer! float!` or `percent!`. 
If `<type-and-size>` are omitted, type is deduced from the contents of `<block>`, and size is
the default size (32 for `char!` and `integer!`, 64 otherwise). If `<block>` is empty, the default type
is `integer!`.
...
....
If `<image-spec>` is `<pair>`, the image is created with the given dimensions, and all pixels having color
...
<bitset-spec> ::= <single-bit> | <string> | [<bit-position>*]
<single-bit> ::= <integer> | <char>
<bit-position> ::= <single-bit> | <string> | <single-bit> - <single-bit> 
....
The built-in function `charset` is defined as shorthand for `make bitset!`.
The `<bitset-spec>` provides a list of bit-position numbers that are to be set to `true`.
The `<integer>` should be non-negative. The `<char>` is interpreted as the Unicode Codepoint number.
A `<string>` is interpreted as the collection of all its component characters. In the block
of `<bit-position>` s two values separated by a `-` are interpreted as a range of numbers.
The length of the bitset is computed as the smallest multiple of 8 needed to fit the highest
bit number (0-origin). An "empty" bitset is 8 bits (one byte) long.

Examples: `charset "abc", charset [#"A" - #"Z" #"a" - #"z"]`
