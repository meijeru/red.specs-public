...
. Values of all types may be represented using `make`, except those of `native! action!` and `routine!`
. _**See issue #2618**_
. _**See issue #2527**_
....
TBD
...
A `date!` value without time specified is different from a `date!` value with zero
time and timezone values, but only as far as displaying the value is concerned.
...
* Making a `url!` value from a `block!` value: the components (at least one is required)
...
including -- if the last one is an `issue!` value -- a _fragment_._**see issues #2900 and #2901**_
...
in this case, over- and under-flow are allowed and taken into account _**see issue #2891**_.
...
in actual value._**See also issues #2658 and #2661**_
...
the three equality tests will all yield `false` and the `not-equal?` test will yield `true`.
