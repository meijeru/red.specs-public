...
`load` will accept binary data in the indicated format and decode it to yield an `image!`
...
A similar facility is offered for comma separated values; the argument to `/as` is `csv`,
the MIME type is ``text/csv` and the extension is `%.csv`.
