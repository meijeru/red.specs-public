...
|`tuple!`         |e.g. colors in RGB or other scheme, IPv4 addresses
...
with the related conversion of data between various representations. Red can work directly
with external resources represented as files (values of type `file!`, which includes
directories or folders as they may be called) and as _Uniform Resource Locators_
or urls (values of type `url!`). For more general handling of resources, Red has _ports_
(values of type `port!`).
...
NB This describes the current state of implementation, which may evolve.

==== Port components

Ports are values of type `port!` that can be used to manage communication
with external resources; they are specialized objects with a fixed number of fields,
that contain the necessary information for handling the communication
with a wide variety of such resources. A prototype port value is contained in `system/standard/port`.
As also shown in <<components-of-values,section 5.6>> the fields are:
[cols="1,1,2", options="header"]
|===
|name      |type           |content
|spec      |object!        |represents a resource, i.e. a URL
|scheme    |object!        |contains _actor functions_
|actor     |               |      
|awake     |               |
|state     |               |
|data      |               |
...
Each _actor function_ handles one particular operation on the resource,
e.g. `open`, `read`, `rename`. These operations are specific to the "type"
of resource, e.g. they may be different for `http:` than for `ftp:`.
The word _scheme_ is used both for the resource type, and for the collection
of actor functions handling this particular resource type.

The fields of the `spec` object are:
[cols="1,1,2", options="header"]
|===
|name      |type           |content
|scheme    |word!          |e.g. http:, ftp:
|user-info |               |
...
A prototype object is contained in `system/standard/url-parts`

A function exists to construct the `spec` object for a URL given a Red value
of type `url!` or `string!`: `decode-url`. It follows https://www.ietf.org/rfc/rfc3986.txt[RFC3986],
which defines "Uniform Resource Identifiers (URI)", with some simplifications.
It will be seen that the field names are identical to those used in RFC3986.

The `scheme` field of the `spec` object names one of the available schemes
stored in `system/schemes`, and this one is the content of the `scheme` field of the
`port!` value. New schemes may be added to `system/schemes` by calling
`register-scheme` with as arguments the name of the scheme and its content.
...
These functions are implemented for the various schemes by actors.

Also defined for values of `port!` type are all functions that are
defined for values of `series` types.
...
system/   object!
    ...
    console/    object!               console program + data
        prompt         string!        see <a href = "#console-functions">section 11.3.3</a>
        result         string!        see <a href = "#console-functions">section 11.3.3</a>
...
Uniform Resource Identifier (URI)       <a href = "#port-components">11.4.1</a>
UTF-8 scheme                            <a href = "#textual-structure">3.1</a>, <a href = "#target-types-in-code-any-string-code">5.4.3</a>, <a href = "#target-type-code-char-code">5.4.7</a>, <a href = "#target-type-code-binary-code">5.4.10</a>, <a href = "#conversion-from-to-textual-representation">11.1.1</a>, <a href = "#file-and-url-i-o">11.3.1</a>
...
issue  section
 ...
 <a href="https://github.com/red/REP/issues/11">11</a>    <a href="#console-functions">11.3.3</a>
