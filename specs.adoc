...
is not true: some direct types are also composite. Hoever, component selection
...
expressions of the form `make <type> <spec>`, where `<type>` evaluates to a type
(`datatype!` value) or to a value ("prototype") of the desired type and `<spec>`
is interpreted by the `make` function as appropriate for the given type.
...
result of the `make` function. This section deals with the types
...
section _**Creation, conversion and casting of values**_.
...
corresponding to the given key. In the case of `object!` and `error!` it is
customary to call the keys "fields".
...
This section treats several related means that Red provides in order to produce
new values:
...
Here `<type>` is a type or example value ("prototype") indicated the
desired type. The significance of `<spec>` for the `make` function has
been explained in section _**Values to be constructed by the make function**_.
For `to` and `as`, `<spec>` is the value to be converted resp. to be given
a new type.

In quite a number of cases, the operation of `make` and `to` is identical.
That is why the sub-section on `make` will only treat the cases where `make`
is different from `to`; also, the cases already treated in section
_**Values to be constructed by the make function**_ will not be repeated.
...
* `any-float!` and `time!` values: truncates the float value (seconds in the case
of `time!`) towards `0`
* `char!` values: yields the Unicode Code Point number
* `binary!` values: interprets the first 4 components as an integer (two's complement notation)
if there are fewer than 4 components, `#{00}` bytes are prepended
...
* `binary!` values: interprets the first 8 components as a floating point number (IEEE 754 binary64 format)
if there are fewer than 8 components, `#{00}` bytes are prepended
* `string!` values: yields the result of `load` or an error
* `block!` values: the block should contain two components of type `integer!` or `float!`
...
* `binary!` values: decodes as many UTF-8 bytes as necessary to obtain a Unicode Code Point
...
and `any-string!`. The type of the second argument should be in the same group
as the (type of the) first argument. The result is a new value of the desired type,
pointing to the components of the old value. Note the absence of `hash!` from the first group,
explained by the fact that `hash!` values are stored differently from other `any-block!` values.
...
`<key>` or `<value>` in a `<map-spec>`. As a consequence, the predefined words `true false`
...
the generalized value construction syntax: `#[true] #[false]` and `#[none]`.
