...
The following functions enable conditional execution of a block:
...
With refinement `/all`, all conditions of the `<case>` are tested; otherwise only
the first one that is satisfied, if any, is executed.
...
to the head of the series, and moving to the `next` on subsequent iterations.
