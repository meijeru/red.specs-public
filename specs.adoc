...
use the value obtained as `<selector>` and go to the beginning of step 3.
.. If the `<selector>` is an `<integer>`, and the composite type is not `map!`
or `any-object!`, an intermediate result is obtained which is the position
...
`<integer>` and the composite type is `any-object!` the path is in error.
.. If the `<selector>` is a `<word>`, and the evaluated `<path-head>` is of
indirect type (except `image!`), an intermediate result is obtained which is
the position of the first component found by applying the built-in function `find`
with as arguments the evaluated `<path-head>` and `<selector>`.
.. If the `<selector>` is a `<word>`, and the evaluated `<path-head>` is of
direct type, `image!` or `event!`, an intermediate result is obtained which is the
position of the first component found as as explained in section _**Selection by "key"**_.
.. If a further `<selector>` is present, use the result just obtained as 
evaluated `<path-head>` and go to step 2, otherwise finish:
replace the component at the position found by the value obtained by evaluating the next expression.
