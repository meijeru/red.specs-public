...
A `date!` value comprises day, month and year information as well as, optionally,
a time of day -- a non-negative `time!` value from `0:00:00` up to `24:00:00` --
optionally combined with timezone information; the latter is a signed time value
between `-15:45` and `+15:45`. Note that actual timezones vary between
`-12:00` and `+14:00`.

A `<date-literal>` uses either a very restricted form of the _ISO 8601 standard
date/time representation_ or a native Red format, as explained in the following
lexical/syntactic rule.

**`S135`**::
....
<date-literal> ::= <ISO 8601 date/time representation> | <date-part><time-part>°
<date-part> ::= <year>/<month>/<day> | <year>-<month>-<day> |
                <day>/month/<year> | <day>-<month>-<year>
<time-part> ::=  /<hmsd><zone>
               | T<hmsd><zone>
<zone>::= Z | +<zone-offset> | -<zone-offset>
<zone-offset> ::= <hours>:<minutes> | <hhmm>
....

Here `<ISO 8601 date/time representation>` is `yyyymmddThhmmssZ`,
with `y` `m` `d` `h` `m` `s` standing for the digits of year, month, day, hours,
minutes and seconds respectively, and `T` and `Z` being obligatory elements.
This represents a date and time with timezone 0:00 (UTC).  The time part is
in the 24 hours system and is used modulo `24:00` with overflow resulting
in increasing the date.

The `<year>` part is either a 4-digit number with -- in initial position --
an optional `-` sign, or -- but only in final position -- an unsigned 1- or 2-digit number
which is interpreted in the current (21^st^) century unless that would make the date
lie in the future, in which case it is interpreted in the previous (20^th^) century.
...
The `<day>` part is a 1- or 2-digit unsigned number. The combination of year, month and day
is checked for validity.

For `<hmsd>` see rule `S113`. The time part is in the 24 hours system and is used modulo `24:00`
with overflow resulting in increasing the date. For `<hours>` and `<minutes>` see again  rule `S113`;
while `<hours>` may be a 1- or 2-digit number, `<minutes>` must have 2 digits here _**see issue #2854**_;
`<hhmm>` stands for hours and minutes as in the ISO representation.
The timezone value is used modulo `16:00` and is rounded down towards zero in multiples of `00:15`.
...
value, the value it refers to will be retrieved. The first 3 components should be of `integer!` type and are interpreted
as year, month, day. If there are 4 or 5 components, the 4th should be a `time!` value for the time of day;
a 5th component should be a `time!` value for the timezone. If there are 6 or 7 components,
the 4th, 5th and 6th components should be hours (`integer!`), minutes (`integer!`), seconds  (`integer!` or `float!`)
for the time of day; a 7th component should be a `time!` value for the timezone.
* For the following types, `make` operates in the same way as `to` (see next section): `pair!` _**to be completed**_
...
The function `to-date` is defined for non-negative _** not checked!**_ `integer!` values
which are interpreted as the number of seconds elapsed since 0:00:00 hours on 1 January 1970 to
the desired date and time (Unix convention). It is also trivially defined for `date!` values.
...
Values of type `time! date! pair!` and `tuple!` also admit component selection
...
For `date!` values, which also have a time component, similarly `1 ~ year 2 ~ month 3 ~ day
4 ~ zone 5 ~ time 6 ~ hour 7 ~ minute 8 ~ second 9 ~ weekday 10 ~ yearday = julian`.
Here `year month day hour minute weekday` and `yearday` (also called `julian`) are `integer!`
values, `second` is a `float!` value and `zone` and `time` are `time!` values.
...
* `date!` values: the `year month day zone time hour minute` and `second` components are extracted
either directly or as in the case of `time!`; the `weekday` component is a number between 1 and 7
(1 = Monday) and the `yearday` component is the ordinal number of the date in the
current year (1 = first of January)
* `email!` values: the `user` component is the part before the `@` (of type `string!`)
and the `host` component is the part after the `@`
...
date!        1 .. 10      pick        year month day zone time           --
                                      hour minute second
...
interpretation                          2.2
ipv4                                    5.2.4
ISO 8601 date/time representation       5.2.14
