...
The built-in functions for indexing are `pick` for extraction and `poke` for changing.
...
and `v/i: x` is equivalent to `poke v i x`. Note that for lexical reasons, a `char!`, `string!`
or `pair!` value used as index must be enclosed in parentheses, and `logic!` values are not allowed.
