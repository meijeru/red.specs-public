...
In the following rules, the sign `°` signifies an optional element,and `^+^` signifies one or more elements.
...
bit number (0-origin). An "empty" bitset is 8 bits (one byte) long on creation.
...
<argument> ::= <word> <argument-doc>° | <word> [<typeset-element>^+^] <argument-doc>°
...
Example: `f: func [a b][all [a b]] &&: make op! :f`.
