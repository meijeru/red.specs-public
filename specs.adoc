...
For `image!` values, a `pair!` is allowed as `<value>` argument to `at` and `skip`.
This will be converted to a regular index. For all series, if the index becomes
smaller than 1 or greater than the length + 1, it will be fixed at 1 or length + 1.
...
The functions `break continue exit return` are descripbed in section _**Exceptions**_.

The functions `halt` and `quit` or `q` stop the execution of the program. _**Difference??_**
