...
The timezone may be represented as in rule `S113`, with `<hours>` being a 1- or 2-digit number,
...
<vector-spec> ::= <integer> | <float> | <block> | [ <type-and-size> <block>]
<type-and-size> ::= char! 8 | char! 16 | char! 32 |
                    integer! 8 | integer! 16 | integer 32! |
                    float! 32 | float! 64 | percent! 32 | percent! 64
....
The `<integer>` should be non-negative. It produces a `vector!` value with the number of components
given by the `<integer>` allocated. These will be of type `integer!` and size 32, and set to `0`.
Likewise, the `<float>` should be non-negative. It produces a`vector!` value with the number of components
given by the truncated `<float>` allocated . These will be of type `float!` and size 32 and set to `0.0`.
_**see issue #2940**_
...
(see section 5.3.12) except that in this case, over- and under-flow are allowed
...
setting the `time` component to `none` will effective set the `time` and `timezone` components
to `0:00` but will also ensure that these components will not be displayed
...
by invoking the built-in function `do` (see section 7.6).
...
(see section 11.1) will be invoked on the string, and `do` will be invoked
on the resulting block
* if the argument is a `url!` or `file!` value, this should contain a text
which is a valid Red program (see section 3); the text string will be
obtained by `read` (see section 11.5) and the previous step will be taken;
...
* `exists-thru?` operates the same with respect to the _thru-cache_ (see section 11.5)  
...
indicated by its `url!` argument (see section 11.5)
* `delete` will take a `file!` argument and delete the file indicated;
the result is `true` if the owill reratin was succesful, and `false` otherwise

TBD
indicated by its `url!` argument
....
A    rename
....
=== Console functions
...
system/   object!
    ...
    options/    object!             
        ...
        thru-cache     file!         see section 11.5
        args           
        do-arg         
        debug          logic!
        secure         
